...
var scaleOptions = []prompt.Suggest{
	...
	prompt.Suggest{Text: "--timeout", Description: "The length of time to wait before giving up on a scale operation, zero means don't wait. Any other values should contain a corresponding time unit (e.g. 1s, 2m, 3h). Usage:\n  kubectl scale [--resource-version=version] [--current-replicas=count] --replicas=COUNT (-f FILENAME | TYPE NAME) [options] Use \"kubectl options\" for a list of global command-line options (applies to all commands)."},
}
